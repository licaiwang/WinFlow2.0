...
from winflow.generator.editor.document import FlowDocument, _job_key, flow_to_document
...
class TestJobGraph(unittest.TestCase):
    ...
    def test_position_changes_preserve_job_order_and_edges(self):
        ...
        for i in range(6):
            name = f"job_{i}"
            out = f"out/{name}.done"
            jobs.append(make_job(name, f"./run {name}", [prev] if prev else [], [out], "q", 1))
            prev = out
        stage = make_stage("stage", [make_task("task", jobs)])
        doc = FlowDocument(flow_name="demo", stages=[stage])
        original_names = [job["name"] for job in stage["tasks"][0]["jobs"]]
        graph_before = build_job_graph(doc)

        keys = [_job_key("stage", "task", name) for name in original_names]
