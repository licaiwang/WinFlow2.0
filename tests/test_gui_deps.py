...
from winflow.generator.core.models import make_job, make_stage, make_task
from winflow.generator.editor.deps import (
...
from winflow.generator.editor.document import FlowDocument, _job_key
from winflow.generator.editor.graph import build_job_graph


def _chain_doc(count: int = 5) -> FlowDocument:
    """Linear job chain in one stage/task — used to exercise link/unlink."""
    jobs = []
    prev_out = None
    for i in range(count):
        name = f"job_{i}"
        outputs = [f"out/{name}.done"]
        jobs.append(
            make_job(
                name,
                f"./run {name}",
                [prev_out] if prev_out else [],
                outputs,
                "q",
                1,
            )
    ...
    return FlowDocument(
        flow_name="demo",
        stages=[make_stage("stage", [make_task("task", jobs)])],
    )


class TestJobDeps(unittest.TestCase):
    def _chain_doc(self) -> FlowDocument:
        return _chain_doc()

    def test_link_and_unlink_jobs(self):
        doc = self._chain_doc()
        jobs = [job["name"] for _s, _t, job in doc.iter_jobs()]
        parent = _job_key("stage", "task", jobs[0])
        child = _job_key("stage", "task", jobs[1])
        ...
        self.assertIn("stage/task/" + jobs[1], doc.get_job(parent)[2]["children"])
    ...
    def test_set_job_parents_multiple(self):
        ...
        j0, j1, j2 = [_job_key("stage", "task", n) for n in names[:3]]
    ...
    def test_rejects_cycle(self):
        from winflow.graph import annotate_job_relations
    ...
    def test_graph_reflects_manual_link(self):
        ...
        parent = _job_key("stage", "task", names[0])
        child = _job_key("stage", "task", names[2])
        err, child, _notes = link_jobs(doc, parent, child)
        self.assertIsNone(err)
        graph = build_job_graph(doc)
