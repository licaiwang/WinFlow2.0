...
def main(argv: Optional[List[str]] = None) -> int:
    """CLI entry: run a flow.json file (default from config)."""
    import sys

    args = list(sys.argv[1:] if argv is None else argv)
    app_config = get_config()
    config_file = args[0] if args else app_config.runner.default_flow_file
