...
_ENV_MAP = {
    ...
    "WINFLOW_EXAMPLE_DEFAULT_QUEUE": ("example", "default_queue"),
    "WINFLOW_EXAMPLE_DEFAULT_CPU": ("example", "default_cpu"),
}


def _project_root() -> Path:
    # winflow/config/loader.py -> repository root
...
def _known_app_config_fields() -> set[str]:
    return {field_info.name for field_info in fields(AppConfig)}


def _store_extra_sections(data: Dict[str, Any]) -> None:
    global _EXTRA_SECTIONS
    known = _known_app_config_fields()
    _EXTRA_SECTIONS = {
...
def get_section(name: str, default: T) -> T:
    """Return a named config section, falling back to *default* when missing.

    Prefer a typed ``AppConfig`` field when present. Otherwise merge any
    matching extra section from ``config.json`` onto *default*. If neither
    exists, return *default* unchanged. Flow authors can therefore define
    defaults locally without editing ``models.py``.
    """
    config = get_config()
    if hasattr(config, name):
        return getattr(config, name)
    raw = _EXTRA_SECTIONS.get(name)
    if isinstance(raw, dict) and is_dataclass(default):
        return merge_dataclass(default, raw)
    return default


def reset_config() -> None:
    global _CONFIG, _EXTRA_SECTIONS
    _CONFIG = None
    _EXTRA_SECTIONS = {}
