...
from winflow.graph import EDGE_TASK_ORDER
from winflow.generator.editor.deps import get_file_parent_keys, get_parent_keys, link_jobs, set_job_parents, unlink_jobs
...
from winflow.generator.editor.graph import JobGraph, build_job_graph
from winflow.generator.editor.nodes import list_nodes_by_flow, load_node, node_dir, node_summary
from winflow.config import get_config
from winflow.icon import apply_window_icon
...
class TemplateLoadDialog(tk.Toplevel):
    ...
    def __init__(self, parent, template_name: str):
        ...
        if self.template_name == "ithome":
            ...
            ttk.Label(
                opts_frame,
                text="When provided, MACHINE_QUEUE / MACHINE_CPU / MACHINE_HOST are read from setting.sh.",
                style="Muted.TLabel",
                wraplength=400,
            ).pack(anchor="w", pady=(0, 8))
            self._file_row(
...
class FlowGeneratorPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        template_values = list_templates()
        ttk.Combobox(
            tools,
            textvariable=self.template_var,
            values=template_values,
            state="readonly",
            width=max(8, max((len(name) for name in template_values), default=8)),
            style="Gen.TCombobox",
