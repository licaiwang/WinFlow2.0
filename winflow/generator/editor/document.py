...
from winflow.generator.core.models import Flow, Job, Stage, Task, make_flow, make_job, make_stage, make_task
from winflow.generator.core.registry import get_builder, list_flows
from winflow.generator.parsers import parse_setting_sh
from winflow.config import get_config

# Import flow packages so @register decorators run before templates are listed.
import winflow.generator.flows  # noqa: F401
...
class FlowDocument:
    ...
    def _relocate_key(self, old_key: JobKey, new_key: JobKey) -> None:
        if old_key == new_key:
            return
        from winflow.graph import key_to_slash, rewrite_relation_key_refs
    ...
    def move_job(
        ...
        if key != new_key:
            from winflow.generator.editor.deps import _rewrite_dummy_paths_for_key
...
def flow_to_document(flow: Flow) -> FlowDocument:
    from winflow.graph import ensure_job_relations
...
def auto_layout_all(document: FlowDocument) -> None:
    from winflow.generator.editor.graph import build_job_graph, layout_by_graph
...
def apply_job_resources(
    ...
    *,
    ...
    for _stage, _task, job in document.iter_jobs():
        job["queue"] = queue
        if apply_cpu and cpu is not None:
            job["cpu"] = int(cpu)
...
def list_templates() -> List[str]:
    """Return template names: blank plus every successfully registered flow."""
    return ["blank", *list_flows()]


def registered_flow_template(
    name: str,
    options: Optional[TemplateOptions] = None,
) -> FlowDocument:
    """Build a template document from a registered FlowBuilder."""
    opts = options or TemplateOptions()
    builder_cls = get_builder(name)
    settings: Dict[str, str] = {
        "MACHINE_QUEUE": opts.queue or DEFAULT_QUEUE,
        "MACHINE_CPU": str(opts.cpu),
        "MACHINE_HOST": opts.machine or "",
    }

    if opts.setting_path and opts.setting_path.exists():
        parsed = parse_setting_sh(opts.setting_path)
        settings.update(parsed)
        # Template dialog resources win when provided.
        settings["MACHINE_QUEUE"] = opts.queue or settings.get("MACHINE_QUEUE", DEFAULT_QUEUE)
        settings["MACHINE_CPU"] = str(opts.cpu)
        if opts.machine:
            settings["MACHINE_HOST"] = opts.machine

    default_setting = (
        Path("example_flow/setting.sh") if name.lower() == "example" else Path("setting.sh")
    )
    context = BuildContext(
        settings=settings,
        blocks=[],
        setting_path=opts.setting_path or default_setting,
    )
    ...
    # ithome sets per-job CPU from yml via cpu_for — do not overwrite.
    apply_job_resources(
        doc,
        opts.queue,
        opts.machine,
        opts.cpu,
        apply_cpu=(name.lower() != "ithome"),
    )
    return doc


def example_template(options: Optional[TemplateOptions] = None) -> FlowDocument:
    return registered_flow_template("example", options)
...
def apply_template(
    ...
    if key == "example_flow":
        key = "example"
    registered = list_flows()
    if key in registered:
        return registered_flow_template(key, opts)
    available = ", ".join(["blank", *registered]) or "blank"
